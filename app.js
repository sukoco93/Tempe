...
new Vue({
  ...
  computed: {
    ...
    cartTotal() { return _.sumBy(this.cart, i => i.qty * i.harga); }
  },
  watch: {
    menu() { this.resetAndLoad(); },
    filter(newVal) {
      if (newVal !== 'range') { this.rangeStart = Utils.today(); this.rangeEnd = Utils.today(); }
      this.resetAndLoad();
    },
    rangeStart() { if (this.filter === 'range') this.resetAndLoad(); },
    rangeEnd() { if (this.filter === 'range') this.resetAndLoad(); }
  },
  methods: {
    // ===== PELANGGAN =====
    getPelangganName(id) { return this.pelangganMap[id]?.nama || 'Umum'; },
    ...
    async loadData(resetOffset = true) {
      ...
      try {
        ...
        if (this.menu === 'kas') {
          this._summary = await db.getKasSummary(this.filter, this.rangeStart, this.rangeEnd);
        }

        // Hilangkan splash screen jika masih ada
        this.hideSplash();
    ...
    resetAndLoad() { this.offset = 0; this.items = []; this.loadData(true); },
    loadMore() {
      if (this.loading) return;
      if (this.items.length >= CONFIG.MAX_VISIBLE_ITEMS) {
    ...
    // ===== SEARCH =====
    onSearchInput: _.debounce(function() { this.resetAndLoad(); }, 300),

    // ===== FILTER =====
    setFilter(val) {
      this.filter = val;
      if (val !== 'range') { this.rangeStart = Utils.today(); this.rangeEnd = Utils.today(); }
      this.resetAndLoad();
    },
    onRangeChange() { if (this.filter === 'range') this.resetAndLoad(); },
    ...
    showToast(msg) {
      ...
      this.toast = msg;
      this.toastTimer = setTimeout(() => { this.toast = ''; this.toastTimer = null; }, 2500);
    },
    ...
    addToCart() {
      ...
      if (!form.barang.trim() || !form.harga) { this.showToast('Isi barang & harga'); return; }
    ...
    async submitForm() {
      ...
      for (const [key, rule] of Object.entries(validateMap)) {
        if (formData[key] !== undefined) {
          ...
          if (result !== true) { this.showToast(result); return; }
        }
      }
      ...
      try {
        await db.transaction('rw', [db.db[menu], db.db.kas], async () => {
          if (menu === 'penjualan') {
            ...
            if (p) {
              const k = await db.db.kas.where({ tgl: p.tgl, nominal: p.total, isAutomated: true }).first();
    ...
    async exportData() {
      ...
      if (!allData.length) { this.showToast('Tidak ada data'); return; }
      ...
      reader.onload = async (ev) => {
        try {
          ...
          for (let i = 1; i < lines.length; i++) {
            ...
            const obj = {};
            headers.forEach((k, idx) => { if (k !== 'id') obj[k] = Utils.toNumber(vals[idx]); });
            bulk.push(obj);
          }
          ...
          this.pelangganMap = await db.getPelangganMap();
          this.resetAndLoad();
        } catch (err) { alert('Eror CSV: ' + err.message); }
      ...
      reader.onload = async (ev) => {
        try {
          ...
          for (const s of ['pelanggan', 'penjualan', 'kas', 'produksi']) {
            if (data[s]) {
              await db.clearAll(s);
              const chunked = data[s].map(item => { delete item.id; return item; });
          ...
          this.pelangganMap = await db.getPelangganMap();
          this.resetAndLoad();
        } catch (err) { alert('JSON rusak: ' + err.message); }
    ...
    // ===== HELPERS =====
    getItemClass(item) {
      if (this.menu === 'kas') return item.jenis === 'masuk' ? 'kas-masuk' : 'kas-keluar';
      return '';
    }
  },
  ...
  mounted() {
    ...
    // Online/offline
    const updateOnline = () => { this.online = navigator.onLine; };
